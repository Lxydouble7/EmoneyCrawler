...
for m in dict:
    ...
    for i in time_set:
        # 数据在XHR里面
        url = 'http://data.eastmoney.com/dataapi/zlsj/list?tkn=eastmoney&ReportDate=' + i + '&code=&type=' + m + '&zjc=0&sortField=Count&sortDirec=1&pageNum=1&pageSize=50&cfg=jjsjtj'
        ...
        csvwriter.writerow(["股票代码", "股票简称", "时间", "持有基金家数", "持股变化", "持股总数", "持股市值", "持股变动数值", "持股变动比例%"])
        # f0：股票代码
        # f1：股票简称
        # f2：时间
        # f3：？？？ eg：基金
...
# for i in time_set:
#     # 数据在XHR里面
#     url = 'http://data.eastmoney.com/dataapi/zlsj/list?tkn=eastmoney&ReportDate=' + i + '&code=&type=1&zjc=0&sortField=Count&sortDirec=1&pageNum=1&pageSize=50&cfg=jjsjtj'
#
#     request = requests.get(url)
#     # 返回的是str
#     # print(request.text)
...
#
#     file_name = '.\data\基金持仓\\' + i + '.csv'
#     file = open(file_name, 'w', encoding='gbk')
#     csvwriter = csv.writer(file)
#
#     csvwriter.writerow(["股票代码", "股票简称", "时间", "持有基金家数", "持股变化", "持股总数", "持股市值", "持股变动数值", "持股变动比例%"])
#     # f0：股票代码
